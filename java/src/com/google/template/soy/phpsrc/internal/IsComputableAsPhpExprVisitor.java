 ...
 * computable as the concatenation of one or more PHP expressions. If this is false, it means the
 * generated code for computing the node's output must include one or more full PHP statements.
 *
 ...
 */
@ApiCallScope
class IsComputableAsPhpExprVisitor extends AbstractReturningSoyNodeVisitor<Boolean> {

    /** The memoized results of past visits to nodes. */
    private final Map<SoyNode, Boolean> memoizedResults;

    @Inject
    IsComputableAsPhpExprVisitor(ErrorReporter errorReporter) {
        super(errorReporter);
     ...
     * computable as PhpExprs. Ignores whether the given node itself is computable as PhpExprs or not.
     */
    public Boolean execOnChildren(ParentSoyNode<?> node) {
        return areChildrenComputableAsPhpExprs(node);
    }
    ...
    @Override protected Boolean visitIfNode(IfNode node) {
        // If all children are computable as PHP expressions, then this 'if' statement can be written
        // as an expression as well, using the ternary conditional operator ("x ? 'a' : 'b'").
        return areChildrenComputableAsPhpExprs(node);
    }
     ...
     * IsComputableAsPhpExprVisitor. ExprNode children are assumed to be computable as PhpExprs.
     *
     * @param node The parent node whose children to check.
     * @return True if all children satisfy IsComputableAsPyExprVisitor.
     */
    private boolean areChildrenComputableAsPhpExprs(ParentSoyNode<?> node) {
