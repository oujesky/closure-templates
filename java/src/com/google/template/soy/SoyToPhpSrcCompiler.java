...
/**
 * Executable for compiling a set of Soy files into corresponding PHP source files.
 *
 */
public final class SoyToPhpSrcCompiler {

  /** The string to prepend to the usage message. */
  private static final String USAGE_PREFIX =
      "Usage:\n"
      + "java com.google.template.soy.SoyToPhpSrcCompiler  \\\n"
...
//  private String runtimePath = "";
  ...
  @Option(name = "--outputPathFormat",
          ...
          usage = "[Required] A format string that specifies how to build the path to each"
                  + " output file. There will be one output PHP file (UTF-8) for each input Soy"
                  ...
                  + " outputted filename outside of the final php extension.")
  private String outputPathFormat = "";

  @Option(name = "--translationClass",
          usage = "The full class name of the PHP translation class."
              + " The name should include the absolute namespace path and class name in \\ notation"
              + " format (e.g. \"my\\package\\module\\TranslatorClass\")."
              + " It is required for {msg} command.")
  private String translationClass = "SimpleTranslator";

  @Option(name = "--bidiIsRtlFn",
          usage = "The full name of a function used to determine if bidi is rtl for setting global"
                  + " directionality. The name should include the absolute namespace path and function"
                  + "name in \\ notation format (e.g. \"my\\app\\bidi\\is_rtl\"). Only applicable if"
  ...
  @Option(name = "--cssHandlingScheme",
          usage = "The scheme to use for handling 'css' commands. Specifying 'literal' will"
                  ...
                  + " The 'goog' scheme is not supported in PHP. This option has no effect if"
  ...
  /**
   * Compiles a set of Soy files into corresponding PHP source files.
   *
  ...
  public static void main(final String[] args) throws IOException, SoySyntaxException {
    MainClassUtils.run(new Main() {
      ...
      public CompilationResult main() throws IOException {
        return new SoyToPhpSrcCompiler().execMain(args);
      }
    });
  }


  private SoyToPhpSrcCompiler() {}
...
//    }
    ...
    // Disallow external call entirely in PHP.
    sfsBuilder.setAllowExternalCalls(false);
    // Require strict templates in PHP.
    ...
    // Create SoyPhpSrcOptions.
    SoyPhpSrcOptions phpSrcOptions = new SoyPhpSrcOptions(/*runtimePath, */bidiIsRtlFn, translationClass);

    // Compile.
    return sfs.compileToPhpSrcFiles(outputPathFormat, inputPrefix, phpSrcOptions);
  }
}
